...
for(int i=7; i<strlen(address.url.data); i++)
{
	...
	new_address.url.data[i] = address.url.data[i+1];
}
...
// To test use UA_STRING("opc.tcp://192.168.1.22:5672")
    UA_Connection connection = UA_ClientConnectionTCP_init( conf, UA_STRING("opc.tcp://192.168.1.22:5672"), 1000, NULL);
...
printf("after set nonblocking socket \n");
    /* Copy UA_Connection */
