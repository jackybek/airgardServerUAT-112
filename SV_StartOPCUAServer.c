...
#define SSLCERTIFICATELOC "/etc/ssl/certs/cert112.pem"	//"/etc/ssl/certs/62541ServerCert.pem"
...
#define GDS_DISCOVERY_SERVER_ENDPPOINT "opc.tcp://192.168.1.44:4841"
...
void RegisterOPCServerToLDS(UA_Server *uaServer1)
{
	UA_StatusCode retval;
		...
		LDSClient_config1->connectivityCheckInterval = 0;
	        ...
	        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "SV_StartOPCUAServer (LDS) : Calling setDefaultEncryption");
...
int GenerateSelfSignedSSLCert(UA_ByteString *privateKey, UA_ByteString *certificate)
{
                        ...
                        UA_String subject[4] = {UA_STRING_STATIC("C=SG"),
                        ...
                        UA_String subjectAltName[3] = {
                                                UA_STRING_STATIC("DNS:192.168.1.112"),//UA_STRING_STATIC("DNS:localhost"),
                                                UA_STRING_STATIC("IP:192.168.1.112"),
...
void* StartOPCUAServer(void* x_void_ptr, char* argv[])
{
	...
	if (g_argc==3 || g_argc==5 || g_argc==6)	// myNewServer 192.168.1.33 192.168.1.88 [192.168.1.11] [1883] pub
	{
		...
		*/
		...
		//	config1.logger = UA_Log_Stdout;
                ...
                char* OPCUAServerIP = OPCipaddress;   // 192.168.1.112
                UA_String hostname;
                UA_String_init(&hostname);
                //hostname.length = strlen(g_argv[1]);
                //hostname.data = (UA_Byte *) g_argv[1];
	...
	*/
		...
		RegisterOPCServerToLDS(uaServer1);
			...
			UA_ServerNetworkLayer *wsconfig = &config1.networkLayers[index];
