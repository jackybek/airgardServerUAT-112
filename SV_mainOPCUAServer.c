...
// using open62541.c
sudo gcc -g -std=c99 -v -I/home/pi/open62541/ -I/home/pi/open62541/include/ -I/home/pi/open62541/plugins/ -lm -lrt -lmodbus \
-lqpid-proton-core -lqpid-proton-proactor -lqpid-proton -lwebsockets -ljson-c -lmariadbclient -lpthread -lxml2 \
-lcrypto -lssl -lmbedcrypto -lmbedtls -lmbedx509 -L/usr/local/lib/ \
open62541.c ua_amqp_adaptor.c ua_pubsub_amqp.c \
SV_Misc.c SV_ExtractXMLElementNames.c SV_Method.c SV_Alarm.c SV_Event.c SV_Monitor.c SV_WebSockets.c SV_Historizing.c SV_PopulateOPCUANodes.c \
SV_CreateOPCUANodes.c SV_StartOPCUAServer.c json_checker.c SV_PubSub.c SV_Modbus.c SV_mainOPCUAServer.c \
-o myNewServer >& error-msg

./myNewServer 192.168.1.33 192.168.1.88 192.168.1.11 1883 pub

touch listings
sudo chgrp pi listings
sudo chown pi listings
./valgrind --leak-check=yes --show-leak-kinds=all ./myNewServer 192.168.1.33 192.168.1.119 192.168.1.11 8885 pub >& listings
valgrind --leak-check=yes ./myNewServer 192.168.1.33 192.168.1.119 192.168.1.11 8885 pub
	...
	*/
	char myport[6];
        switch (argc)
        {
			...
			argv[0]= "./myNewServer" ;
                        argv[1] = "192.168.1.112";
                        argv[2] = "192.168.1.119";
                        argv[3] = "192.168.1.20";
                        argv[4] = myport;
                        argv[5] = "sub";
                        argc=6;
	...
	//UA_ServerConfig_setDefault(config);	// do this in StartOPCUAServer.c
        ...
        g_argc = argc;
	strcpy(g_argv_ip, argv[1]);	// 192.168.1.112
