...
#define DATASETWRITERID			2234	// used in addDataSetReader() and addDataSetWriter()
 ...
 #define CLIENT_ID_MQTT               	"OPCServerUAT-112" // during execution, mqtt-11 will show : New client connected from 192.168.1.33 as OPCServer-33-Mqtt (c0, k400, u'jackybek')
 ...
 #define CLIENT_CERT_PATH_MQTT			"/etc/ssl/certs/mosq-client-112.crt"	// symbolic link => /home/pi/Downloads/openest.io/mosq-client-112.crt
 #define CLIENT_KEY_PATH_MQTT			"/etc/ssl/certs/mosq-client-112.key"	// symbolic link => /home/pi/Downloads/openest.io/mosq-client-112.key
 // tested successfully on command line
 // sudo mosquitto_pub -d -h Mqtt-11 -p 8883 -t AirgardTopic --capath /etc/ssl/certs/ --cafile mosq-ca.crt --cert /etc/ssl/certs/brokerserver-11.crt --key /etc/mosquitto/certs/brokerserver-key11.key -m "Secure Message 2" --tls-version tlsv1.2 --insecure
...
*/
    #ifdef DEBUG_MODE
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "CreateServerPubSubSV_PubSub.c : addPubSubConnection() \n");
    #endif
...
// line 236
	...
	// login credentials to mqtt broker on 192.168.1.20
...
--cert /etc/ssl/certs/mosq-client-112.crt 
--key /etc/ssl/certs/mosq-client-112.key -d -i OPCServerUAT-112 -h 192.168.1.20
	...
	if ( brokeraddress != NULL)
	{
		...
		if ( (port == 8883) || (port == 8884) || (port == 8885) || (port == 8887) )
		{
			...
			AMQP_Enable = UA_FALSE;
                        ...
                        strcpy(URI_mqtt, "opc.mqtt://");		//strcpy(URI_mqtt, "opc.mqtt://");
                        strcat(URI_mqtt, brokeraddress);
                        strcat(URI_mqtt, ":");
			itoa(port, portbuf, 10);
			strcat(URI_mqtt, portbuf);
			//strcat(URI_mqtt, "/\0");	// tutorial example does not have "/"
		...
		else if (port == 5672)
		{
			...
			MQTT_TLS_Enable = UA_FALSE;
                        ...
                        strcat(URI_amqp, ":5672");	// tutorial example does not have "/"
