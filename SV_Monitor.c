...
int ActivateWebSocketStreams(const UA_Server *server);
...
//    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "SV_Monitor.c : dataChangeNotificationCallback : Receiving Notification .. data (MonitoredItem) has changed");
	// saves a copy of the data to the Historian
	writeHistoryData(nodeId, value);	// function is declared in SV_Historizing.c
	// push the data to port 7681 websockets
	ActivateWebSocketStreams(server);		// function is declared in SV_WebSockets.c
}
